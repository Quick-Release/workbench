...
import { effortParamFromSearch, focusParamFromSearch } from "../lib/issue-param";
...
describe("the blocker graph view", () => {
  it("renders one node per member and gate plus a warning node per dangling reference", () => {
    const html = renderToString(<BlockersPage {...effort()} />);
  ...
  it("lands the shared panel's show-in-graph link with the node focused", () => {
    // The panel's href (ticket #60) is `/blockers?effort=GH-41&focus=42`;
    // feeding that query through the view's param grammar must select the
    // effort and focus the node.
    const search = Object.fromEntries(new URLSearchParams({ effort: "GH-41", focus: "42" }));
    const effortId = effortParamFromSearch(search);
    const focusId = focusParamFromSearch(search);
    expect(effortId).toBe("GH-41");
    expect(focusId).toBe("GH-42");
    const jumped = effort();
    jumped.effortId = effortId ?? null;
  ...
  it("renders depth column captions and the effort's name", () => {
    const html = renderToString(<BlockersPage {...effort()} />);
  ...
  it("names the maps in the effort picker and flags an effort the snapshot does not carry", () => {
    const html = renderToString(<BlockersPage {...effort()} />);
    expect(html).toContain("GH-41");

    const unknown = effort();
    unknown.effortId = "GH-999";
    const unknownHtml = renderToString(<BlockersPage {...unknown} />);
    expect(unknownHtml).toContain("GH-999");
    expect(unknownHtml).toContain("no such effort");
  });
});
