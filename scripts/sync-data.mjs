...
const configuredSourceRoot = process.env.WORKBENCH_SOURCE_ROOT;
const rootDirectory = resolve(
  configuredSourceRoot || superprojectDirectory || gitDirectory || process.cwd(),
);
...
const main = async () => {
  ...
  const repositoryUrl = process.env.WORKBENCH_REPOSITORY_URL || repositoryWebUrlFrom(remote);
  const repo = repositoryNameFrom(repositoryUrl);
  const projectName = process.env.WORKBENCH_PROJECT_NAME || repo.split("/").pop() || repo;
  ...
  console.log(
    `Synced ${tickets.length} tickets, ${plans.length} plans, and ${changes.length} OpenSpec changes from local Markdown.`,
  );
};
