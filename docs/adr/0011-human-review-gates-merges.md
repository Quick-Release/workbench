...
- The repository's GitHub policy must enforce the path to that human decision: `main` requires a pull request, conversation resolution, and the `CI / validate` check, with the protection applying to administrators (#189). The CI workflow checks out and asserts the pull request's head SHA before running `pnpm check` and `pnpm test`. GitHub approving reviews are deliberately not required: the sole Developer commonly owns agent-authored PRs and cannot approve their own PR through GitHub. GitHub cannot encode the Developer's out-of-band instruction or distinguish an agent using the same account, so the exact-revision human instruction remains a procedural gate. Repository CI is also candidate-controlled and is not ADR 0021's independent verification; workflow/check changes require direct human scrutiny and are ineligible for #167's first automated-publication profile.
