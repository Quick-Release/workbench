...
Scope clarification from ADR 0014 (GH-159): these fixed-prompt, local-model and draft-PR decisions remain the coding issue agent's contract. The separately approved owned-clarification boundary permits proposal correction and an explicitly selected cloud model, ending at an approved issue brief, not code. It is not implemented or authorized for implementation by that decision. A draft PR's review gate does not substitute for approval to start clarification or publish its issue update.
