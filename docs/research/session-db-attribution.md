...
- **Model/agent attribution:** `query_source` (`main_turn` 46 / `subagent` 15 / `session_title` 3 — filter the last, as `scripts/sync/sessions.mjs` does), `agent` (`zcode-agent` 49 / `zcode-general-purpose` 12 / `zcode-Explore` 3), `mode` (`yolo` 64), `provider_id`/`model_id` (single model locally: `builtin:zai-coding-plan / GLM-5.3-Flash`), all joinable on `session_id`.
...
- `scripts/sync/sessions.mjs:35-73` — the copy-then-open-read-only method and the known schema subset this doc extends.
