...
- `@effect/platform-node` appears in the codebase only via the ADR 0003 Alchemy work (commit `9604f0f`), which is telemetry-worker infrastructure, not the research doc's stage 2 (typed errors in `scripts/commands/sync-data.mjs`). As of this date `scripts/commands/sync-data.mjs` imports no Effect (`grep -n "effect" scripts/commands/sync-data.mjs` — no matches beyond unrelated words).
...
- **Every pipeline exercises the gate.** `dev`, `build`, `test`, and `check` all run `pnpm sync` first (`package.json:28,32,35`), regenerating `src/data.generated.ts` (gitignored — not present in a fresh clone; produced by `scripts/commands/sync-data.mjs`) before anything imports it. Validation runs on plain module import, no new Vite/TS/test-runner config — as the spec required.
...
3. If the team still wants the research doc's stage 2 (typed errors in `scripts/commands/sync-data.mjs`), start it as a fresh, current-state issue — the v4-RC baseline and the seam contracts that now live in `src/schema.ts` change the premises the old spec was written against.
...
- Codebase: `src/data.ts:1-5`; `src/schema.ts:25,85,262,277,289,309,377,406-412`; `src/data.test.ts:1,96-140`; `src/routes/index.tsx:7,116`; `package.json:28,32,35,41,51,54,59,62,63`; `scripts/commands/sync-data.mjs` (no Effect imports).
