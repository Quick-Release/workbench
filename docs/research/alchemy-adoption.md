...
> **Public-access update (2026-09-12):** This research predates ADR 0013. References to Workbench's internal distribution and private-era release state are historical; the current reporting posture is internal provisioning with public installs dormant by omission.
...
export const Database = Cloudflare.D1.Database("workbench-telemetry", {
  migrations: "./worker/migrations", // initial schema is 0001_init.sql (see below)
});
...
**D1 schema/migrations (historical comparison).** At the time of this research, the schema was applied once by hand with `npx wrangler d1 execute workbench-telemetry --remote --file worker/schema.sql`; the current initial migration is `worker/migrations/0001_init.sql` ([worker/README.md](../../worker/README.md)). Cloudflare's own versioning model would be `wrangler d1 migrations` with a `migrations/` folder and a `d1_migrations` bookkeeping table ([Cloudflare D1 migrations](https://developers.cloudflare.com/d1/reference/migrations/)). Alchemy's model: point `migrations` at "a folder of `.sql` files… sorted by numeric prefix (0001_, 0002_, …) and applied in order as part of every deploy; already-applied migrations are skipped", with bookkeeping in Alchemy's `__alchemy_migrations` table; databases previously migrated with `wrangler d1 migrations apply` are adopted automatically, and "D1 has no transactions over HTTP, so each migration and its bookkeeping row are sent as one batched query, matching wrangler's own behavior" ([D1](https://alchemy.run/cloudflare/data/d1)). Because the pre-Alchemy repo never used `wrangler d1 migrations` (plain `execute --file`) and its original schema was fully idempotent (`CREATE TABLE IF NOT EXISTS` throughout), creating `worker/migrations/0001_init.sql` was a no-op against the live database. One unverified detail: the exact prop carrying `compatibility_date: "2026-08-01"` — Alchemy's Workers page says a Worker carries "name, bundle, compatibility flags", but I did not find the prop name documented on the pages fetched.
...
- Repo files read: `package.json`, `pnpm-workspace.yaml`, `worker/wrangler.jsonc`, `worker/worker.mjs`, `worker/migrations/0001_init.sql`, `worker/worker.test.mjs`, `worker/README.md`, `.github/workflows/release.yml`, `CONTEXT.md`, `docs/adr/0001-mandatory-telemetry-internal-tool.md`
