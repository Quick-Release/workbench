...
// class and mounts the Agents SDK's routes and the session-capture proxy
// strictly after the same constant-time bearer-token check that guards
// the ingest routes — neither surface is ever wider than the ingest API
// (tickets #31 and #35). The ingest handler itself stays runtime-free in
// ingest.mjs, where the node --test suite drives it directly.
